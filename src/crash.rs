...
pub const CRASH_GAME_ID: u8 = 6;
pub const POW2_32: u64 = 4_294_967_296;
pub const RTP_PERCENT: u32 = 99;
/// Minimum crash point (×100). Crash CAN land at 1.00x — everyone loses.
pub const MIN_CRASH_X100: u64 = 100;
...
/// Crash payout divisor: cashout_x100 / 100 (not ×10000 like other games).
pub const CRASH_PAYOUT_DIVISOR: u64 = 100;

/// Compute the crash point (×100) from Poseidon2 random output via inverse transform sampling.
///
///   v_lo  = random[0] & 0xFFFF_FFFF  (lower 32 bits)
///   denom = 2^32 - v_lo              (range 1..=2^32)
///   raw   = floor(RTP_PERCENT × 2^32 / denom)
///   result = clamp(raw, MIN_CRASH_X100, MAX_MULTI_X100)
///
/// When raw < MIN_CRASH_X100 (≈1% of rounds), crash point = 1.00x and all
/// users lose because minimum cashout is 1.01x.
pub fn crash_point_from_random(random: &[u64; 4]) -> u64 {
    let v_lo = random[0] & 0xFFFF_FFFF;
    let denom = POW2_32 - v_lo;
    let raw = (RTP_PERCENT as u128 * POW2_32 as u128) / denom as u128;
    (raw as u64).clamp(MIN_CRASH_X100, MAX_MULTI_X100)
}

/// Full payout computation for Crash — pure integer arithmetic, zero floats.
///
...
///   - 0 = user did NOT cash out (loss — rode the rocket into the crash).
///   - 101..=MAX_MULTI_X100 = user stopped at this multiplier (min 1.01x).
///
/// Win condition: `cashout_x100 > 0 && cashout_x100 <= crash_point`.
/// Payout on win: `min(bet × cashout_x100 / CRASH_PAYOUT_DIVISOR, MAX_WIN)`.
pub fn compute_payout(
    ...
    let won = cashout_x100 > 0 && (cashout_x100 as u64) <= crash_x100;

    let (win_amount, multiplier) = if won {
        let raw = (bet_atomic as u128 * cashout_x100 as u128) / CRASH_PAYOUT_DIVISOR as u128;
